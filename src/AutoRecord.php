...
class AutoRecord {
    ...
    public static final function loadRow(AutoDb $autoDb, $table, $keyname = null, $value = null)
    {
        ...
        if (AutoDb::isPgsqlResource($sqlr)) {
            $sqlGet = "SELECT * FROM " . pg_escape_string($sqlr, $table) .
    ...
    public function dbAttrForce($column)
    {
        ...
        if (AutoDb::isPgsqlResource($sqlr)) {
            $sql = 'SELECT ' . pg_escape_string($sqlr, $column) . ' FROM ' . pg_escape_string($autoDb->getSqlResource(), $this->_tableName) .
                ' WHERE ' . $this->getPrimaryKey() . ' = ' . (int)$this->getPrimaryKeyValue();
    ...
    public function escape($value)
    {
        ...
        if (AutoDb::isPgsqlResource($this->_sqlResource)) {
            return pg_escape_string($this->getSqlResource(), $value);
        }
    ...
    public final function forceReloadAttributes()
    {
        ...
        if (AutoDb::isPgsqlResource($sqlr)) {
            $sqlGet = "SELECT * FROM " . pg_escape_string($sqlr, $this->getTableName()) .
                " WHERE " . pg_escape_string($sqlr, $this->getPrimaryKey()) . " = " . (int)$this->getPrimaryKeyValue();
    ...
    {
        $sqlr = $this->_sqlResource;
        $sql = 'INSERT INTO ' . pg_escape_string($sqlr, $this->getTableName()). ' ';
        ...
        foreach ($this->_rowChanged as $row) {
            ...
            $colNames .= pg_escape_string($sqlr, $row);
            $values .= $this->_getCommasAndEscapes($row, $this->_attributes[$row]);
        }
    ...
    {
        $sqlr = $this->_sqlResource;
        $sql = 'UPDATE ' . pg_escape_string($sqlr, $this->getTableName()) . ' SET ';
    ...
    private static function _saveCheckedArrayOptimised(array $toInsert, $sqlr, $insertCommand, $suffix)
    {
        ...
        foreach ($toInsert as $autoRecord) {
            ...
            if (AutoDb::isPgsqlResource($sqlr)) {
                // set columns if first run
                if ($insertQuery === '') {
                    $insertQuery = $insertCommand . ' ' . pg_escape_string($sqlr, $autoRecord->getTableName()) . ' ';
                    ...
                    foreach ($autoRecord->_columnRules as $key => $rules) {
                        ...
                        $colNames .= pg_escape_string($sqlr, $key);
                    }
                    $insertQuery .= "( $colNames ) VALUES ";
                }
            }
    ...
    public static final function deleteMore(array $arrayOfAutoRecords)
    {
        ...
        if (AutoDb::isPgsqlResource($sqlr)) {
            $deleteQuery = 'DELETE FROM ' . pg_escape_string($sqlr, $tablename) . ' WHERE '
                . pg_escape_string($sqlr, $primaryKey) . ' IN (' . implode(',', $deleteIds) . ')';
